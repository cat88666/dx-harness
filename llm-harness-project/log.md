...
## [2026-04-22] Update | 统一 llm-harness-project 到主题聚合规则
...
## [2026-04-22] Update | 清理 llm-harness-project 的旧 demo 内容
...
- 新建目录：`llm-harness-project/cleaned/course/transcript/`
- 处理文件：全部 14 个 transcript .txt 文件
- 修复项：
  - `自助理机制` → `自注意力机制`（共 50 处，分布于 7 个文件）
  - `自助理` → `自注意力`（涵盖全部剩余用例）
  - `代研码` → `带掩码`（Transformer-2 和 MLA 文件各 2 处）
- 更新 `llm-harness-project/CLAUDE.md`：记录 cleaned/ 目录约定及 ingest 优先级

## [2026-04-27] Update | cleaned/ 深度去噪：语义修复 + 口语重复去除

- 操作范围：`llm-harness-project/cleaned/course/transcript/` 全部 14 个文件
